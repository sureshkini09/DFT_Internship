...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller (
  ...
  output wire [1:0] BIST_COL_ADD, // column address
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_add_format (
  input  wire [1:0] Y_ADDRESS,
  input  wire [2:0] X_ADDRESS,
  output wire [1:0] COLUMN_ADDRESS,
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_add_gen (
  ...
  output wire A_EQUALS_B_TRIGGER
    ...
    wire   [1:0]     Y_R, Y_RP, Y_RCO, Y_RCI;
    reg    [1:0]     NEXT_Y_ADD_COUNT;
    wire   [1:0]     Y_ADD_CNT;
    wire   [1:0]     Y_ADD_CNT_TO_BUF;
    wire   [1:0]     Y_ADD_CNT_MAX, Y_ADD_CNT_MIN;
    wire   [0:0]     Y_ADD_ROT_OUT;
    ...
    wire   [1:0]     Y_ADD_REG_MIN_DEFAULT;
    wire   [1:0]     Y_ADD_REG_MAX_DEFAULT;
    wire   [0:0]     Y_ADD_ROT_OUT_DEFAULT;
...
//------------------------------
    ...
    assign Y_RP      = (({2{INCY1}} & (~Y_R)) | ({2{DECY1}} & Y_R));
 
    assign Y_RCO     = (Y_RCI | Y_RP) ;
 
    assign Y_RCI     = (~Y1_CNT_EN) ? {2{1'b1}}                                                                :
                                      ({Y_RCO[0:0], 1'b0 })                                     ;
...
//-----------------------------------------------------------------
    ...
    case (X_ADD_ROT_IN_SELECTED) 
     2'b00 : begin
                     AX_ROT_IN_SELECTED =     AY_ADD_REG[1] ;
     end
     2'b01 : begin
    ...
    case (X_ADD_ROT_IN_SELECTED) 
     2'b00 : begin
                     BX_ROT_IN_SELECTED =     BY_ADD_REG[1] ;
           end
     2'b01 : begin
...
//----------------------------------------------
    // Algorithm: SMARCH Type: IC
    assign Y_ADD_REG_MIN_DEFAULT    = 2'b00;
 
    assign Y_ADD_CNT_MIN            = Y_ADD_REG_MIN_DEFAULT;
 
    // Algorithm: SMARCH Type: IC
    assign Y_ADD_REG_MAX_DEFAULT    = MBISTPG_REDUCED_ADDR_CNT_EN ? 2'b11 : 2'b11;
...
//----------------------------------------
    assign Y_ADD_ROT_OUT_DEFAULT    = 1'b1;
...
//------------------------------------
    assign           AY_ADD_WIRE    = 2'b00; // Algorithm: SMARCH
 
    assign AY_MASK   =                      (BIST_SHIFT_SHORT)    ? 2'b11 :
                      (Y_ADD_ROT_OUT == 1'b1)      ? 2'b11 :
                                                                    2'b01;
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        ...
        if (BIST_SHIFT_SHORT | (LAST_TICK & (~BIST_HOLD) & ROT_LF_A_ADD_REG & BIST_RUN))
            AY_ADD_REG              <= (AY_MASK & {AY_ADD_REG[0:0], AY_ROT_IN_SELECTED}) | ((~AY_MASK) & AY_ADD_REG);
        else
        if (LAST_TICK & (~BIST_HOLD) & ROT_RT_A_ADD_REG & BIST_RUN)
            AY_ADD_REG              <= {AY_ROT_RT_IN_SELECTED, AY_ADD_REG[1:1]};
        else begin
          if ( ENABLE_A_ADD_REG & BIST_RUN & (~BIST_HOLD))
              if (LAST_TICK & (~INH_LAST_ADDR_CNT)) begin
...
//------------------------------------
    assign           BY_ADD_WIRE    = 2'b00; // Algorithm: SMARCH
 
    assign BY_MASK   =                      (BIST_SHIFT_SHORT)    ? 2'b11 :
                      (Y_ADD_ROT_OUT == 1'b1)      ? 2'b11 :
                                                                    2'b01;
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        ...
        if (BIST_SHIFT_SHORT | (LAST_TICK & (~BIST_HOLD) & ROT_LF_B_ADD_REG & BIST_RUN))
            BY_ADD_REG              <= (BY_MASK & {BY_ADD_REG[0:0], BY_ROT_IN_SELECTED}) | ((~BY_MASK) & BY_ADD_REG);
        else begin
          if ( ENABLE_B_ADD_REG & BIST_RUN & (~BIST_HOLD))
              if (LAST_TICK & (~INH_LAST_ADDR_CNT)) begin
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_data_gen (
  ...
  output wire SO
    ...
    wire   [1:0]     Y_ADDR_BIT_SEL_DECODED;
